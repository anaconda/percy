...
if [[ ! -f argon2_cffi-feedstock.mark ]]; then
    if [[ -d argon2_cffi-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./argon2_cffi-feedstock >d 2>&1 && rm -f d && ( echo "done" >>argon2_cffi-feedstock.mark ) && true) || ( (echo "argon2_cffi-feedstock" >>failed.19 ) && (echo "argon2_cffi-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "argon2_cffi-feedstock not present" >>failed.19
    fi
fi
