...
if [[ ! -f mkl_random-feedstock.mark ]]; then
    if [[ -d mkl_random-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./mkl_random-feedstock >d 2>&1 && rm -f d && ( echo "done" >>mkl_random-feedstock.mark ) && true) || ( (echo "mkl_random-feedstock" >>failed.23 ) && (echo "mkl_random-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "mkl_random-feedstock not present" >>failed.23
    fi
fi
...
if [[ ! -f pyobjc-core-feedstock.mark ]]; then
    if [[ -d pyobjc-core-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./pyobjc-core-feedstock >d 2>&1 && rm -f d && ( echo "done" >>pyobjc-core-feedstock.mark ) && true) || ( (echo "pyobjc-core-feedstock" >>failed.23 ) && (echo "pyobjc-core-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "pyobjc-core-feedstock not present" >>failed.23
    fi
fi
