...
if [[ ! -f cppy-feedstock.mark ]]; then
    if [[ -d cppy-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./cppy-feedstock >d 2>&1 && rm -f d && ( echo "done" >>cppy-feedstock.mark ) && true) || ( (echo "cppy-feedstock" >>failed.18 ) && (echo "cppy-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "cppy-feedstock not present" >>failed.18
    fi
fi
...
if [[ ! -f dbus-python-feedstock.mark ]]; then
    if [[ -d dbus-python-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./dbus-python-feedstock >d 2>&1 && rm -f d && ( echo "done" >>dbus-python-feedstock.mark ) && true) || ( (echo "dbus-python-feedstock" >>failed.18 ) && (echo "dbus-python-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "dbus-python-feedstock not present" >>failed.18
    fi
fi
...
if [[ ! -f pyqt-builder-feedstock.mark ]]; then
    if [[ -d pyqt-builder-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./pyqt-builder-feedstock >d 2>&1 && rm -f d && ( echo "done" >>pyqt-builder-feedstock.mark ) && true) || ( (echo "pyqt-builder-feedstock" >>failed.18 ) && (echo "pyqt-builder-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "pyqt-builder-feedstock not present" >>failed.18
    fi
fi
...
if [[ ! -f pyqt-feedstock.mark ]]; then
    if [[ -d pyqt-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./pyqt-feedstock >d 2>&1 && rm -f d && ( echo "done" >>pyqt-feedstock.mark ) && true) || ( (echo "pyqt-feedstock" >>failed.18 ) && (echo "pyqt-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "pyqt-feedstock not present" >>failed.18
    fi
fi
