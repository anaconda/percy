...
if [[ ! -f tensorboard.mark ]]; then
    if [[ -d tensorboard ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./tensorflow_recipes/tensorboard >d 2>&1 && rm -f d && ( echo "done" >>tensorboard.mark ) && true) || ( (echo "tensorboard" >>failed.25 ) && (echo "tensorboard" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "tensorboard not present" >>failed.25
    fi
fi
