...
if [[ ! -f pyzmq-feedstock.mark ]]; then
    if [[ -d pyzmq-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./pyzmq-feedstock >d 2>&1 && rm -f d && ( echo "done" >>pyzmq-feedstock.mark ) && true) || ( (echo "pyzmq-feedstock" >>failed.14 ) && (echo "pyzmq-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "pyzmq-feedstock not present" >>failed.14
    fi
fi
