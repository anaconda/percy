...
if [[ ! -f pyinotify-feedstock.mark ]]; then
    if [[ -d pyinotify-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./pyinotify-feedstock >d 2>&1 && rm -f d && ( echo "done" >>pyinotify-feedstock.mark ) && true) || ( (echo "pyinotify-feedstock" >>failed.23 ) && (echo "pyinotify-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "pyinotify-feedstock not present" >>failed.23
    fi
fi
