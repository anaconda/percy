...
if [[ ! -f dask-searchcv-feedstock.mark ]]; then
    if [[ -d dask-searchcv-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./dask-searchcv-feedstock >d 2>&1 && rm -f d && ( echo "done" >>dask-searchcv-feedstock.mark ) && true) || ( (echo "dask-searchcv-feedstock" >>failed.29 ) && (echo "dask-searchcv-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "dask-searchcv-feedstock not present" >>failed.29
    fi
fi
