...
if [[ ! -f nose-feedstock.mark ]]; then
    if [[ -d nose-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./nose-feedstock >d 2>&1 && rm -f d && ( echo "done" >>nose-feedstock.mark ) && true) || ( (echo "nose-feedstock" >>failed.15 ) && (echo "nose-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "nose-feedstock not present" >>failed.15
    fi
fi
