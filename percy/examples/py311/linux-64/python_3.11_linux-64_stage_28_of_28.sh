...
if [[ ! -f dash-bio-feedstock.mark ]]; then
    if [[ -d dash-bio-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./dash-bio-feedstock >d 2>&1 && rm -f d && ( echo "done" >>dash-bio-feedstock.mark ) && true) || ( (echo "dash-bio-feedstock" >>failed.28 ) && (echo "dash-bio-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "dash-bio-feedstock not present" >>failed.28
    fi
fi
...
if [[ ! -f dropbox-feedstock.mark ]]; then
    if [[ -d dropbox-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./dropbox-feedstock >d 2>&1 && rm -f d && ( echo "done" >>dropbox-feedstock.mark ) && true) || ( (echo "dropbox-feedstock" >>failed.28 ) && (echo "dropbox-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "dropbox-feedstock not present" >>failed.28
    fi
fi
...
if [[ ! -f quilt3-feedstock.mark ]]; then
    if [[ -d quilt3-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./quilt-feedstock >d 2>&1 && rm -f d && ( echo "done" >>quilt3-feedstock.mark ) && true) || ( (echo "quilt3-feedstock" >>failed.28 ) && (echo "quilt3-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "quilt3-feedstock not present" >>failed.28
    fi
fi
