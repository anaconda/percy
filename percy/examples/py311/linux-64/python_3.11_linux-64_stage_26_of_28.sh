...
if [[ ! -f flask-compress-feedstock.mark ]]; then
    if [[ -d flask-compress-feedstock ]]; then
        (conda-build --python=3.11 --numpy=1.22 --croot=../ci_py311/ -c py311_bs/label/release --use-local --no-test ./flask-compress-feedstock >d 2>&1 && rm -f d && ( echo "done" >>flask-compress-feedstock.mark ) && true) || ( (echo "flask-compress-feedstock" >>failed.26 ) && (echo "flask-compress-feedstock" >>errors.dump ) && ( cat d >>errors.dump ) && cat d && rm -f d && true) || true
    else
        echo "flask-compress-feedstock not present" >>failed.26
    fi
fi
